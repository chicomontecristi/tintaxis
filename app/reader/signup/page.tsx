...
function ReaderSignupInner() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/reader/signup", {
      ...
      body:    JSON.stringify({ email, password, name, readingName: readingName || name }),
    });
    ...
    >
      ...
      >
        ...
        <p style={{
          ...
          Your annotations will be remembered.
        </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: "1.25rem" }}>
            <label style={labelStyle}>
              Reading Name{" "}
              <span style={{ color: "rgba(201,168,76,0.25)", fontSize: "0.4rem" }}>
                — how you appear in the margins
              </span>
            </label>
            <input
              type="text"
              ...
              placeholder={name || "e.g. IronLibrarian_9"}
              autoComplete="off"
              style={inputStyle}
            />
          </div>
          ...
          <div style={{ marginBottom: "1.75rem" }}>
            <label style={labelStyle}>
              Password{" "}
              <span style={{ color: "rgba(201,168,76,0.25)", fontSize: "0.4rem" }}>— min 8 characters</span>
            </label>
            <input
              type="password"
              value={password}
          ...
          >
            {loading ? "Creating your archive..." : "Claim Your Archive →"}
          </motion.button>
        </form>
        ...
        <p style={{
          ...
          Already have access?{" "}
          <Link
            href={`/reader/login${returnTo !== "/" ? `?from=${encodeURIComponent(returnTo)}` : ""}`}
